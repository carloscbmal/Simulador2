...
def simulador_promocao():
    ...
    # Corrigido: Guardamos o valor do slider em uma variável clara
    tempo_aposentadoria_user = st.sidebar.slider(
    ...
    if botao_simular:
        ...
        for idx_ciclo, data_referencia in enumerate(datas_ciclo):
            ...
            # Máscara de quem deve se aposentar agora
            mask_apo = (idade >= 63) | (servico >= tempo_aposentadoria_user)
            
            if mask_apo.any():
                militares_aposentando = df[mask_apo]
                
                # Registrar no histórico das matrículas foco que estão se aposentando
                for m_foco in matriculas_foco:
                    if m_foco in militares_aposentando['Matricula'].values:
                        historicos[m_foco].append(f"🛑 {data_referencia.strftime('%d/%m/%Y')}: APOSENTADO")
                
                # Mover para inativos e remover do DF principal
        ...
        for i, m_foco in enumerate(matriculas_foco):
            with abas[i]:
                ...
                # Verifica status final no DF de ativos
